(function() {
    ...
    var configData = {
        searchPluginStatus: '0', 
        sectionsDataJSON: '[{\"status\":\"承認１\",\"emailTitle\":\"This is confirmation mail for submission\",\"emailBody\":\"Thank you for submit your improvement report\",\"multiFields\":\"氏名, 社員番号, 作成日時, ステータス, 改善タイトル, 改善前, 改善後4\"},{\"status\":\"承認2\",\"emailTitle\":\"Approval Confirmation\",\"emailBody\":\"Your improvement report is approved by authority\",\"multiFields\":\"レコード番号, 改善タイトル, 改善前, 改善後\"}]'
    }

    const emailConfigureData = JSON.parse(configData.sectionsDataJSON);
    //console.log(emailConfigureData);
    //console.log(emailConfigureData.length)
    ...
    async function emailFormatter(record) {
        ...
        try {
            ...
            if (emailAddress !== '') {
                ...
                if (!emailConfig) {
                    console.log("No matching email config found.");
    ...
    async function sendEmailHandaler(getRecord) {
        ...
        if (emailDetails.body !== '' || emailDetails.title !== '') {
            ...
            // console.log(emailDetails.title);
            // console.log(emailDetails.emailAddress);
            console.log("Here");
